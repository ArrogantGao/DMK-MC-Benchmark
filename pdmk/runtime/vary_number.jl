...
function main()
    ...
    filename = "pdmk_runtime.csv"
    CSV.write(filename, DataFrame(N = Int[], L = Float64[], time_propose = Float64[], time_accept = Float64[]))

    Ns = [10000, 12689, 16102, 20433, 25929, 32903, 41753, 52983, 67233, 85316, 108263, 137382, 174332, 221221, 280721, 356224, 452035, 573615, 727895, 923670, 1172102, 1487352, 1887391, 2395026, 3039195, 3856620, 4893900, 6210169, 7880462, 10000000]
    ...
    for N in Ns
        ...
        # warm up
        eval_shift_energy(tree, 1, 0.1, 0.1, 0.1)
        update_shift!(tree, 1, 0.1, 0.1, 0.1)
