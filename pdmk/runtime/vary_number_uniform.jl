...
function main()
    ...
    filename = joinpath(@__DIR__, "pdmk_runtime_uniform_float.csv")
    CSV.write(filename, DataFrame(N = Int[], L = Float64[], ns = Int[], n_levels = Int[], time_propose = Float64[], time_accept = Float64[]))

    Ns = [10000, 12689, 16102, 20433, 25929, 32903, 41753, 52983, 67233, 85316, 108263, 137382, 174332, 221221, 280721, 356224, 452036, 573615, 727895, 923670, 1172100, 1487352, 1887391, 2395026, 3039195, 3856622, 4893900, 6210169, 7880462, 10000000]
    nss = [200]
