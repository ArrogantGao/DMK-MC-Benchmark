...
def timing_uniform(bc, method, N, N_propose=1000, N_accept=1000, R=None, local=False):
...
def timing_nonuniform(bc, method, N, N_propose=1000, N_accept=1000, R=None, local=False):
    ...
    for i in range(N):
        r = np.sqrt(pi[i, 0]**2 + pi[i, 1]**2 + pi[i, 2]**2)
        pi[i, 0] = (0.25 * e) * pi[i, 0] / r
        pi[i, 1] = (0.25 * e) * pi[i, 1] / r
        pi[i, 2] = (0.25 * e) * pi[i, 2] / r
